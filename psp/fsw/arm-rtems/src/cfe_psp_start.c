...
void filesystem_test (const char* base);
...
void goCFE(void)
{
   CFE_PSP_Main(2, "/cf/cfe_es_startup.scr");
}

/*
** Reboot command -- Jump to the start of PROM
*/
int reboot_command( int argc, char *argv[])
{
   FuncPtr_t             FunctionPointer = 0x0;   
...
void CFE_PSP_Main(  int ModeId, char *StartupFilePath )
{
   ...
   */ 
   rtems_shell_add_cmd("reboot","misc","reboot system",reboot_command);
   ...
   */
   ...
   CFE_PSP_ReservedMemoryPtr = (CFE_PSP_ReservedMemory_t *)0x00586000;
